...
mask = gradcam.attribute(input_tensor, target=0)
mask_upsampled = LayerAttribution.interpolate(mask, input_tensor.shape[2:])
heatmap = np.abs(mask_upsampled.squeeze().cpu().detach().numpy())

# -------- NORMALIZE & CREATE RGBA HEATMAP --------
# normalize to [0,1]
heat_min, heat_max = heatmap.min(), heatmap.max()
heatmap_norm = (heatmap - heat_min) / (heat_max - heat_min + 1e-8)
# map to green RGBA
greens_cmap = cm.get_cmap('Greens')
rgba_heatmap = greens_cmap(heatmap_norm)
# set full transparency where heatmap == 0
alpha_val = 0.6
rgba_heatmap[..., 3] = np.where(heatmap > 0, alpha_val, 0)
...
# optional colorbar showing intensity scale
